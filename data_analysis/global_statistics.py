...
def run_global_statistics(output: str = "data/global_statistics.json") -> None:
    ...
    """
    graph_container = construct_container(
        course_file="data/courses.json",
        program_file="data/programs.json",
        department_file="data/glossary.json",
        breadth_file="data/breadths.json",
    )
    ...
    courses_filtered_by_cr_ncr = separate_courses_by_cr_ncr_eligibility(
        graph_container.graph
    )

    stats["Total number of courses eligible for CR/NCR"] = len(
        courses_filtered_by_cr_ncr[True]
    )
    stats["Total number of courses not eligible for CR/NCR"] = len(
    ...
    courses_filtered_by_breadth = separate_courses_by_breadth(
        graph_container.graph, list(graph_container.breadths.keys())
    )
    ...
    stats["Average number of direct prerequisites"] = (
        graph_container.graph.avg_num_direct_requisites()
    )

    stats["Average number of total prerequisites"] = get_avg_total_num_prereqs(
        graph_container.graph
    )
