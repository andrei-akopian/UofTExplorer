...
def separate_courses_by_cr_ncr_eligibility(
    graph: CourseGraph,
) -> list[dict[str, CourseNode]]:
    """
...
def separate_courses_by_breadth(
    graph: CourseGraph, breadth_categories: list[str]
) -> list[dict[str, CourseNode]]:
    """
    ...
    return graph.get_filtered_courses(
        ...
        [
            lambda course, b=breadth: course.is_in_breadth(b)
            for breadth in breadth_categories
        ],
        breadth_categories,
    )


def separate_courses_by_department(
    graph: CourseGraph, departments: dict[str, str]
    ...
    conditions = [
        lambda course, d=dept_code: course.is_in_department(d)
...
def get_prereq_course_set(
    graph: CourseGraph, origin: str | CourseNode, max_depth: int = -10
) -> set[str]:
    """
...
def get_postreq_course_set(
    graph: CourseGraph, origins: list[str | CourseNode]
) -> set[str]:
    """
...
def get_filtered_graph(
    container: CourseGraphContainer, query: str, filters: dict[str, list[str]]
) -> dict[str, list | str]:
    """
    ...
    if query_lower == "all":
        ...
        subgraph = construct_subgraph(
            container.graph,
            list(
                container.graph.get_filtered_courses(
    ...
    # Includes the course's prerequisites
        ...
        subgraph = construct_subgraph(
            container.graph, [query_upper], traversers.Targets(True, True, False, False)
        )
        graph_data["search"] = query_upper
        graph_data["curr_query"] = {
            "type": "course",
            ...
            [
                lambda course: all(
                    [
                        filters["cr_ncr"] == []
                        or any(
                            course.matches_cr_ncr(elig) for elig in filters["cr_ncr"]
                        ),
                        filters["departments"] == []
                        or any(
                            course.is_in_department(dept)
                            for dept in filters["departments"]
                        ),
                        filters["breadth"] == []
                        or any(
                            course.is_in_breadth(breadth)
                            for breadth in filters["breadth"]
                        ),
                    ]
                )
            ]
...
def get_search_suggestions(
    ...
    # If program matches is shorter than maximum, show extra courses
        ...
        matches.extend(
            ...
            + course_matches[
                : max_results["courses"]
                + (max_results["programs"] - len(program_matches))
            ]
        )
    ...
    # If course matches is shorter than maximum, show extra programs
        ...
        matches.extend(
            program_matches[
                : max_results["programs"]
                + (max_results["courses"] - len(course_matches))
            ]
            + course_matches
        )
    ...
    # Otherwise, both are longer than maximum, so limit both
        ...
        matches.extend(
            program_matches[: max_results["programs"]]
            + course_matches[: max_results["courses"]]
        )
...
def get_program_suggestions(
    ...
    filtered_programs = container.get_filtered_programs(
        lambda program_obj: program_obj.code_contains(query)
        or program_obj.title_contains(query)
    )
...
def compute_live_statistics(
    graph: CourseGraph, graph_container: CourseGraphContainer
) -> dict[str, str | int]:
    """
    ...
    stats["Number of courses eligible for CR/NCR"] = len(
        courses_filtered_by_cr_ncr[True]
    )
    stats["Number of courses not eligible for CR/NCR"] = len(
        courses_filtered_by_cr_ncr[False]
    )

    # Filter courses by breadth requirement
    courses_filtered_by_breadth = separate_courses_by_breadth(
        graph, list(graph_container.breadths.keys())
    )
