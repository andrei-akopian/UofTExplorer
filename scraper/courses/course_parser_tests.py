...
class MyTestCase(unittest.TestCase):
    ...
    def test_req_string_to_opslist(self):
        cp = CourseParser()
        # -- trivial cases --
        assert cp.req_string_to_opslist("MAT240H1/ MAT240H5") == [
            "MAT240H1",
            "/",
            "MAT240H5",
        ]
        ...
        assert parsed == ["(", "MAT157H5", ",", "MAT159H5", ")", ",", "CSC110H1"]
        parsed = cp.req_string_to_opslist("(MAT157H5, MAT159H5)/ CSC110H1")
        assert parsed == ["(", "MAT157H5", ",", "MAT159H5", ")", "/", "CSC110H1"]
        parsed = cp.req_string_to_opslist(
        ...
        parsed = cp.req_string_to_opslist(
            "MAT135H1/ MAT136H1/ MAT137Y1/ MAT157Y1, ( CSC108H1/ equivalent programming experience)/ CSC110Y1/ CSC148H1 *Note: the corequisite may be completed either concurrently or in advance."
        )
        assert parsed == [
            "MAT135H1",
            "/",
        ...
        # JFL388H1
        parsed = cp.req_string_to_opslist(
            "4.0 credits, and any 100-level or higher language course OR introductory linguistics course (e.g., LIN200H1, FRE272H1, ITA360H1, SLA323H1/ SLA380H1, SPA322H1)"
        )
        assert parsed == [
            "(",
            "LIN200H1",
    ...
    def test_nested_lists_flattener(self):
        ...
        parsed = cp.nested_lists_flattener(
            ["AND", ["OR", "A", "C", ["OR", ["AND", "D"], "E"]], "B"]
        )
    ...
    def test_rpn_parser(self):
        cp = CourseParser()
        parsed = cp.requisites_to_polish_notation(["MAT157Y1", "/", "MAT137Y1"])
    ...
    def test_rpn_to_nested_lists(self):
        cp = CourseParser()
        parsed = cp.requisites_pn_to_nested_lists(["MAT157Y1", "MAT137Y1", "/"])
        ...
        assert parsed == [
            ...
            [
                "AND",
                "ABC444H1",
                ["AND", ["OR", "ABC555H1", "ABC666H1"], ["OR", "ABC555H1", "ABC666H1"]],
            ],
        ]
    ...
    def test_simplify_requisite(self):
        ...
        output = cp.simplify_requisite(
            ["AND", ["OR", "D", "B", "C"], "G"], {"A", "B", "C"}
        )
        assert output == ["OR", "B", "C"]
        output = cp.simplify_requisite(
            ["AND", ["OR", "D", ["AND", "B", "C"]], "G"], {"A", "B", "C"}
        )
